...
function Get-SPDiagnosticFarmBuildInfo
{
    ...
    $finding = New-SPDiagnosticFinding -Name "Farm Info" -Description "Farm build and config db" -InputObject $retObj -Format List -Expand 
    $ulsPath = $(get-spdiagnosticconfig).LogLocation
    $finding.Description += "<br>The ULS logs are located at the following path:"
    $finding.Description += '<ul style="color:#0072c6">' + $ulsPath + '</ul>'
    return $finding
...
{
    $serverColl = @()
    foreach($svr in (Get-SPServer | Sort-Object Role, Name))
    {
...
function Get-SPDiagnosticsSSPJobInstances
{
    ...
    $farm = [Microsoft.SharePoint.Administration.SPFarm]::Local
    $farmSspJob = $farm.Services | Where-Object {$_.TypeName -like "SSP Job Control*"}
